...
bool FPL_ScaledAnimRootMotionLayeredMove::GenerateMove(const FMoverTickStartData& SimState,
...
bool FPL_ScaledAnimRootMotionLayeredMove::HasBlockingPawnCollision(const UMoverComponent* MoverComp, const FVector& WorldTranslation) const
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(PLRootMotionCollisionStop), false, OwnerActor);
...
bool FPL_ScaledAnimRootMotionLayeredMove::IsRelevantCollisionHit(const FHitResult& Hit, const AActor* OwnerActor) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
{
	FPL_ScaledAnimRootMotionLayeredMove* CopyPtr = new FPL_ScaledAnimRootMotionLayeredMove(*this);
	return CopyPtr;
}
...
{
	return FString::Printf(TEXT("PL_ScaledAnimRootMotion"));
}
